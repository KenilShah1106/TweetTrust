...
@section('title', 'Edit Replies')

@section('hero-header-left')
    <h2 class="hero-title m-0">Edit a Replies</h2>
@endsection

@section('hero-body')
    <div class="bg-white p16 br-10">
        <form action="{{route('tweets.replies.update',[$tweet->id,$answer->id])}}" class="m-0" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
